...
class TestClusterTools(unittest.TestCase):
    ...
    def test_list_clusters_handles_noise(self, mock_get_all_clusters):
        ...
        mock_get_all_clusters.return_value = [
            {"id": "noise", "name": "Noise Cluster", "size": 15}
        ]
    ...
    def test_get_cluster_with_members(self, mock_get_cluster, mock_get_chunks):
        ...
        result = tools.get_cluster("cluster-1", include_members=True, member_limit=20)
    ...
    @patch("mcp_server.tools.firestore_client.get_cluster_by_id")
    def test_search_within_cluster_success(
        self, mock_get_cluster, mock_search, mock_generate_embedding
    ):
        """Test search_within_cluster filters to cluster members (AC #3)."""
        mock_get_cluster.return_value = {"id": "cluster-1", "name": "Productivity"}
        ...
        result = tools.search_within_cluster_tool(
            "cluster-1", "focus techniques", limit=10
        )
...
class TestEnhancedSearchTools(unittest.TestCase):
    ...
    @patch("mcp_server.tools.firestore_client.find_nearest")
    @patch("mcp_server.tools.firestore_client.get_cluster_by_id")
    def test_search_semantic_includes_knowledge_card_and_cluster(
        self, mock_get_cluster, mock_find_nearest, mock_generate_embedding
    ):
...
class TestEdgeCases(unittest.TestCase):
    ...
    @patch("mcp_server.tools.firestore_client.find_nearest")
    @patch("mcp_server.tools.embeddings.generate_query_embedding")
    def test_missing_cluster_handled_gracefully(
        self, mock_generate_embedding, mock_find_nearest
    ):
        ...
        mock_find_nearest.return_value = [
            {
                ...
                "chunk_index": 0,
                "total_chunks": 1,
                "knowledge_card": {"summary": "Has summary", "takeaways": []},
                # No cluster_id field
            }
        ]
