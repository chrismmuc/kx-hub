...
2. **Domain Whitelist**: Firestore config at `config/recommendation_domains` - used as trusted sources boost (not filter)
...
7. **Unit Tests**: 20 tests created, all 20 pass
8. **Documentation**: Updated mcp-server-usage.md with full tool documentation and example conversations
9. **KB Credibility Signals** (`get_kb_credibility_signals()`): Extracts all authors (top 100) and source domains (top 50) from entire KB for credibility scoring
10. **Credibility Scoring**: +0.5 for known authors, +0.3 for known source domains, +0.5 for trusted public sources
11. **Recency Filtering**: Articles older than 90 days filtered out; recency_score (0-1) boosts recent articles
12. **Advanced Search**: Tavily `search_depth="advanced"` for better quality ranking

### Remaining Items

- ~~Create Tavily API account~~ Done - API key stored in Secret Manager
- ~~Integration testing~~ Done - validated with real API
- ~~Manual validation in Claude Desktop~~ Done
- Performance: ~40-55s (acceptable for quality)
