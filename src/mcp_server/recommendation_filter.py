...
from src.llm import BaseLLMClient, get_client
...
def calculate_combined_score(
    ...
    # Apply adjustments
    adjustments = {"novelty_bonus": novelty_bonus, "domain_penalty": domain_penalty}
...
def diversified_sample(
    ...
    try:
        ...
        indices = np.random.choice(
            len(results), size=min(n, len(results)), replace=False, p=probabilities
        )
...
def assign_slots(
    recommendations: List[Dict[str, Any]], slot_config: Optional[Dict[str, int]] = None
) -> List[Dict[str, Any]]:
    """
    ...
    by_recency = sorted(
        recommendations, key=lambda x: x.get("recency_score", 0), reverse=True
    )
    ...
    for rec in by_combined:
        if (
            len([r for r in result if r.get("slot") == SlotType.RELEVANCE])
            >= relevance_count
        ):
            break
        if rec.get("url") not in assigned_urls:
            ...
            rec["slot_reason"] = (
                f"Top combined score ({rec.get('final_score', rec.get('combined_score', 0)):.2f})"
            )
            result.append(rec)
    ...
    for rec in by_combined[mid_start:]:
        if (
            len([r for r in result if r.get("slot") == SlotType.STALE_REFRESH])
            >= stale_count
        ):
            break
    ...
    for rec in by_recency:
        if (
            len([r for r in result if r.get("slot") == SlotType.TRENDING])
            >= trending_count
        ):
            break
        if rec.get("url") not in assigned_urls:
...
def generate_why_recommended(
    recommendation: Dict[str, Any], query_context: Dict[str, Any]
) -> str:
    """
    ...
    try:
        ...
        if source == "cluster":
            ...
            if takeaway:
                short_takeaway = (
                    takeaway[:50] + "..." if len(takeaway) > 50 else takeaway
                )
...
def check_kb_duplicate(
    title: str, content: str, url: str = None, author: str = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if len(stripped_title) >= 3:
            similar_by_title = firestore_client.find_chunks_by_title_prefix(
                stripped_title, limit=5
            )
            ...
            for chunk in similar_by_title:
                ...
                if (
                    stripped_title in kb_core
                    or kb_core in stripped_title
                    or stripped_title in kb_title
                    or kb_title in title_lower
                ):
                    logger.debug(
                        f"Duplicate detected by title containment: '{title}' ~ '{kb_title}'"
                    )
                    return {
                        "is_duplicate": True,
        ...
        if author and len(author) > 2:
            ...
            author_chunks = firestore_client.find_chunks_by_title_prefix(
                stripped_title[:10], limit=20
            )
            ...
            for chunk in author_chunks:
                ...
                if author_lower in chunk_author or chunk_author in author_lower:
                    ...
                    if len(overlap) >= 1:  # At least one significant word in common
                        logger.debug(
                            f"Duplicate detected by author match: {author} wrote '{chunk_title}'"
                        )
                        return {
                            "is_duplicate": True,
        ...
        similar_chunks = firestore_client.find_nearest(
            embedding_vector=embedding, limit=1
        )
        ...
        if similar_chunks:
            ...
            if word_similarity > 0.4:
                logger.debug(
                    f"Duplicate detected by embedding+title: similarity={word_similarity:.2f}"
                )
                return {
                    "is_duplicate": True,
        ...
        for rec in recommendations:
            ...
            if published_date_str:
                try:
                    ...
                    for fmt in ("%Y-%m-%d", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%dT%H:%M:%SZ"):
                        try:
                            article_date = datetime.strptime(
                                published_date_str[:19], fmt[: len(published_date_str)]
                            )
                            break
                        except ValueError:
                            continue
                    ...
                    if article_date and cutoff_date and article_date < cutoff_date:
                        ...
                        logger.debug(
                            f"Filtered (too old {published_date_str}): {title[:50]}..."
                        )
                        continue
            ...
            if check_duplicates:
                ...
                if dup_check.get("is_duplicate"):
                    ...
                    logger.debug(
                        f"Filtered (duplicate via {match_type}): '{title[:40]}' ~ '{similar_title}'"
                    )
                    continue
...
def batch_score_content(
    ...
    try:
        ...
        for i in range(0, len(items), batch_size):
            ...
            # Build batch prompt
            batch_prompt = (
                "Rate the depth and quality of these articles on a scale of 1-5:\n\n"
            )
...
[{"article": 1, "score": <1-5>}, {"article": 2, "score": <1-5>}, ...]"""
            ...
            try:
                ...
                for score_item in scores:
                    ...
                    if 0 <= idx < len(batch):
                        batch[idx]["depth_score"] = max(
                            1, min(5, score_item.get("score", 3))
                        )
