# Story 3.2: Automated Periodic Re-clustering with Drift Detection

## Status: PLANNED (Defer Until Scale Justifies)
**Epic:** 3 - Advanced Knowledge Graph & System Optimization
**Dependencies:** Story 2.3 (Clustering Consistency Fix) ✅ COMPLETE
**Priority:** LOW (Quality optimization, manual process sufficient for 15-30 months)

**📚 Reference:** See [Clustering Reclustering Strategy](../clustering-reclustering-strategy.md) for detailed strategic analysis
...
- ✅ Works well for current 835 chunks
- ✅ Quarterly run takes ~5 minutes
- ✅ Cost: ~$0.04/year

**This Story (Phase 1):** Automated drift detection + scheduled reclustering
- ⏰ Implement when: Corpus hits 5,000+ chunks OR growth >200/month OR manual process breaks
- 📅 Timeline: Not needed for 15-30 months (current growth: ~50-100/month)
- 💰 Cost: ~$0.07/month (when implemented)
- ⚙️ Effort: 2-3 weeks
...
3. **Track metrics:**
   ...
   - When corpus hits 2,500 chunks → revisit this story

4. **Decision point:**
   - If manual process feels burdensome → implement Phase 1 (this story)
   - If everything runs smoothly → continue Phase 0
...
Initial Clustering (835 chunks):
  └─ UMAP.fit_transform() - Learns manifold structure of 835 chunks
     └─ Creates 2.7 MB model capturing the topology
  └─ Saves "frozen manifold" to Cloud Storage

Delta Processing (new chunks arrive):
  └─ UMAP.transform() - Projects new chunks onto the frozen manifold
     └─ Does NOT update the model
     └─ Assumes new chunks fit the old topology
```
...
| Relative growth | 180%+ (→2338 chunks) | **15-30 months** |
...
def should_trigger_reclustering(db: firestore.Client) -> tuple[bool, str]:
    ...
    if last_size > 0:
        ...
        if growth_rate >= 1.80:  # 180% growth (e.g., 835 → 2338)
...
**Documentation:**
- [ ] Runbook for manual reclustering (Phase 0) ✅ (exists)
...
✅ **Phase 0 is COMPLETE and SUFFICIENT for current scale:**
