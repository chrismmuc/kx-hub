...
**Algorithm: UMAP + HDBSCAN Pipeline**

The script uses a two-stage pipeline (BERTopic standard):
1. **UMAP**: Dimensionality reduction (768D → 5D)
2. **HDBSCAN**: Density-based clustering on reduced embeddings

**Parameters:**
```python
# UMAP parameters
use_umap: True
umap_n_components: 5         # Target dimensionality
umap_n_neighbors: 15         # Local neighborhood size
umap_metric: 'cosine'        # Preserve semantic similarity

# HDBSCAN parameters
min_cluster_size: 10         # Minimum cluster size (prevents over-fragmentation)
min_samples: 3               # Density estimation parameter
hdbscan_metric: 'euclidean'  # On UMAP-reduced space
```
...
### Current Configuration (Optimized with UMAP)

Based on research from BERTopic, HDBSCAN best practices, and semantic clustering literature (2024-2025):

| Parameter | Value | Rationale |
|-----------|-------|-----------|
| `use_umap` | True | Dimensionality reduction prevents curse of dimensionality |
| `umap_n_components` | 5 | Reduces 768D embeddings to 5D, preserving semantic structure |
| `umap_metric` | cosine | Preserves angular distance in embedding space |
| `min_cluster_size` | 10 | Prevents over-fragmentation, creates semantically coherent clusters |
| `min_samples` | 3 | Balanced density estimation for UMAP-reduced space |
| `hdbscan_metric` | euclidean | Optimal for UMAP-reduced embeddings (cosine already applied) |

### Actual Results (UMAP + HDBSCAN)

With 823 chunks and UMAP + HDBSCAN pipeline:

- **Cluster count:** 37 clusters + 1 noise cluster
- **Average cluster size:** 19.9 chunks
- **Median cluster size:** 16 chunks
- **Noise ratio:** 10.6% (87 points) ✅ **Within target 10-20%**
- **Silhouette score:** 0.226 (acceptable for semantic clustering)
- **Semantic coherence:** High (clusters represent distinct topics)

### Evolution of Parameters

**Phase 1: Initial deployment (over-fragmented)**
```python
min_cluster_size: 3
min_samples: 2
metric: cosine (precomputed distance)
```
- Result: 120 clusters, 15.1% noise, mean size 5.8 chunks
- Problem: Over-fragmentation, 37.5% had minimum size

**Phase 2: Parameter tuning (high noise)**
```python
min_cluster_size: 10
min_samples: 3
metric: cosine (precomputed distance)
```
- Result: 21 clusters, **43.9% noise**, mean size 18.8 chunks
- Problem: Curse of dimensionality (768D embeddings) caused excessive noise

**Phase 3: UMAP + HDBSCAN (optimal)**
```python
use_umap: True
umap_n_components: 5
min_cluster_size: 10
...
```
- Result: 37 clusters, **10.6% noise** ✅, mean size 19.9 chunks
- Solution: UMAP dimensionality reduction solved curse of dimensionality

**Research citations:**
- BERTopic standard: UMAP → HDBSCAN pipeline for semantic clustering
- HDBSCAN research: High-dimensional spaces require dimensionality reduction
- UMAP paper: Preserves local and global structure for density-based clustering
...
### Production Metrics (UMAP + HDBSCAN)

```
Total chunks: 823
Clusters: 37 (+ 1 noise)
Noise ratio: 10.6% ✅ (target: 10-20%)
Mean size: 19.9 chunks
Median size: 16 chunks
Min size: 10 chunks
Max size: 79 chunks
Silhouette score: 0.226 (acceptable)
Processing time: ~4 seconds (UMAP: 3.9s, HDBSCAN: 0.1s)
```

### Top 5 Clusters by Size

1. **Effective Change and Personal Growth** (79 chunks)
2. **Introversion and Quiet Strengths** (45 chunks)
3. **Systems, Complexity, and Diversity** (36 chunks)
4. **AI Reshaping Development & Agile** (35 chunks)
5. **AI & Auto Industry Updates** (32 chunks)
...
### Initial Load (823 chunks) - UMAP + HDBSCAN
- **UMAP reduction:** ~3.9 seconds (768D → 5D)
- **HDBSCAN clustering:** ~0.1 seconds (on 5D space)
- **Firestore writes:** ~1.5 seconds (batch)
- **Gemini naming:** ~120 seconds (37 clusters)
...
- [UMAP Documentation](https://umap-learn.readthedocs.io/) - Dimensionality reduction for visualization and clustering
- [HDBSCAN Parameter Selection](https://hdbscan.readthedocs.io/en/latest/parameter_selection.html)
- [BERTopic Clustering Configuration](https://maartengr.github.io/BERTopic/getting_started/clustering/clustering.html) - Standard UMAP → HDBSCAN pipeline
- [UMAP Paper (McInnes et al. 2018)](https://arxiv.org/abs/1802.03426) - Uniform Manifold Approximation and Projection
...
- [Curse of Dimensionality in Clustering](https://arxiv.org/abs/1803.08230) - Why dimensionality reduction improves density-based clustering

### Implementation Files
- `src/clustering/initial_load.py` - Bulk clustering script
- `src/clustering/cluster_metadata.py` - Cluster naming generator
- `src/clustering/clusterer.py` - UMAP + HDBSCAN wrapper
...
- [x] Initial clustering script processes 800+ chunks in < 3 minutes
- [x] UMAP + HDBSCAN pipeline implemented (768D → 5D → clusters)
- [x] Noise ratio within target range (10.6% vs target 10-20%)
- [x] Cluster metadata includes AI-generated names and descriptions (37 clusters)
...
- [x] Clustering parameters optimized based on research (UMAP + HDBSCAN)
