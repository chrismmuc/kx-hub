...
- [ ] Task 1: Research and select clustering algorithm (AC: #2, #10)
  - [ ] Evaluate K-Means vs HDBSCAN vs Hierarchical clustering
  - [ ] Test with sample of 100 chunks using existing 768-dim embeddings
  - [ ] Measure cluster quality (silhouette score, manual inspection)
  - [ ] Document algorithm selection rationale with web research findings
  - [ ] Validate cosine similarity metric for semantic clustering

- [ ] Task 2: Design cluster storage schema (AC: #7, #8)
  - [ ] Define `kb_items.cluster_id` field schema (array of strings)
  - [ ] Design optional `kb_clusters` collection schema (id, label, member_count, created_at)
  - [ ] Define `graph.json` structure for Epic 3 export (nodes, edges, cluster metadata)
  - [ ] Document schema in code comments and architecture docs

- [ ] Task 3: Implement core clustering logic module (AC: #2, #5, #6)
  - [ ] Create `src/clustering/clusterer.py` with reusable clustering logic
  - [ ] Implement cosine similarity distance calculation
  - [ ] Implement selected clustering algorithm (HDBSCAN or K-Means)
  - [ ] Add cluster quality metrics (silhouette score, inertia if K-Means)
  - [ ] Handle edge cases: single-member clusters, noise points (-1 label for HDBSCAN)

- [ ] Task 4: Implement initial load script (AC: #1, #3, #5, #10)
  - [ ] Load all 818 chunks with embeddings from Firestore `kb_items`
  - [ ] Run clustering algorithm on all embeddings
  - [ ] Batch update Firestore with cluster_id assignments (500 updates/batch)
  - [ ] Log progress every 100 chunks
  - [ ] Generate graph.json and upload to Cloud Storage
  - [ ] Implement idempotency: clear existing cluster_id arrays before recomputing
  - [ ] Handle retry logic for Firestore batch write failures

- [ ] Task 5: Implement delta processing Cloud Function (AC: #4, #6, #10)
  - [ ] Create `functions/clustering/main.py` Cloud Function entry point
  - [ ] Accept batch of new chunk IDs via HTTP POST from Cloud Workflows
  - [ ] Load existing cluster centroids/representatives from Firestore
  - [ ] For each new chunk: assign to nearest existing cluster OR create new cluster
  - [ ] Batch update Firestore with new cluster assignments (500 updates/batch)
  - [ ] Regenerate graph.json with updated cluster memberships
  - [ ] Implement error handling and structured logging

- [ ] Task 6: Create Terraform infrastructure (AC: #4)
  - [ ] Define Cloud Function in `terraform/clustering.tf`
  - [ ] Create service account with Firestore read/write + GCS write permissions
  - [ ] Configure function: Python 3.11, europe-west4, 512MB memory, 5-min timeout
  - [ ] Set environment variables: GCP_PROJECT, GCP_REGION, FIRESTORE_COLLECTION
  - [ ] Update `terraform/workflows/batch-pipeline.yaml` to add clustering step after Knowledge Cards

- [ ] Task 7: Testing and validation (AC: #2, #5, #6, #9)
  - [ ] Unit test: clustering algorithm with synthetic embeddings
  - [ ] Unit test: Firestore batch write logic (mocked)
  - [ ] Integration test: Initial load with 50 test chunks
  - [ ] Integration test: Delta processing with 10 new chunks
  - [ ] Manual quality check: Review 20 random clusters for semantic coherence
  - [ ] Performance test: Measure initial load time (target ≤10 min)
  - [ ] Performance test: Measure delta processing time (target ≤5 min)
  - [ ] Cost validation: Verify no new AI API calls, only Firestore/GCS costs

- [ ] Task 8: Documentation and deployment (AC: #8)
  - [ ] Document clustering algorithm choice and parameters in Dev Notes
  - [ ] Update architecture.md with clustering step in pipeline
  - [ ] Create README for src/clustering module
  - [ ] Deploy Cloud Function via `terraform apply`
  - [ ] Run initial load script: `python3 -m src.clustering.initial_load`
  - [ ] Validate graph.json exported to Cloud Storage
