...
- [x] Task 1: Implement recency scoring function (AC: #1)
  - [x] Create `calculate_recency_score(published_date, half_life_days)` in `recommendation_filter.py`
  - [x] Implement exponential decay: `exp(-ln(2) * age_days / half_life_days)`
  - [x] Add `max_age_days` cutoff filter
  - [x] Unit tests for various age scenarios (today, 30d, 90d, 180d, 365d)
  - [x] Handle missing publication dates gracefully (assign neutral score 0.5)

- [x] Task 2: Implement multi-factor ranking (AC: #2, #8)
  - [x] Create `calculate_combined_score(result, weights)` function
  - [x] Normalize all factor scores to 0-1 range
  - [x] Implement weighted sum with configurable weights
  - [x] Add score breakdown to recommendation response
  - [x] Unit tests for score combination

- [x] Task 3: Implement "shown recommendations" tracking (AC: #3, #6)
  - [x] Create Firestore collection `shown_recommendations/{user_id}/items`
  - [x] Document structure: `{url, shown_at, expires_at}`
  - [x] Implement `record_shown(user_id, urls)` in firestore helpers
  - [x] Implement `get_shown_urls(user_id, ttl_days)` query
  - [x] Add TTL-based auto-cleanup (Firestore TTL policy or Cloud Function)
  - [x] Filter shown URLs from results in recommendation engine

- [x] Task 4: Implement stochastic sampling (AC: #3)
  - [x] Create `diversified_sample(results, n, temperature)` function
  - [x] Implement softmax over scores with temperature parameter
  - [x] Temperature 0 = deterministic (top-N), temperature 1 = uniform random
  - [x] Default temperature: 0.3 (mild randomization)
  - [x] Unit tests for sampling distribution

- [x] Task 5: Implement slot-based rotation (AC: #4)
  - [x] Define slot strategy enum: RELEVANCE, SERENDIPITY, STALE_REFRESH, TRENDING
  - [x] Implement `assign_slot(recommendation, slot_type)` logic
  - [x] Create `get_serendipity_cluster()` - find related but unread cluster
  - [x] Create `get_stale_clusters(threshold_days)` - clusters needing refresh
  - [x] Implement slot filling algorithm in main recommendation function

- [x] Task 6: Implement query variation (AC: #5)
  - [x] Create `generate_varied_queries(themes, session_seed)` function
  - [x] Implement cluster rotation based on time/session
  - [x] Add synonym expansion for key terms
  - [x] Create "beyond" query templates using takeaways
  - [x] Unit tests for query variety

- [x] Task 7: Implement configuration management (AC: #7)
  - [x] Create Firestore document `config/ranking_weights`
  - [x] Create Firestore document `config/ranking_settings`
  - [x] Implement `get_ranking_config()` in firestore_client.py
  - [x] Implement `update_ranking_config()` MCP tool
  - [x] Validate weight sum equals 1.0
  - [x] Register tool in main.py

- [x] Task 8: Integrate with recommendation engine (AC: #1-8)
  - [x] Modify `get_reading_recommendations()` to use new ranking
  - [x] Add `exclude_urls` parameter to Tavily searches
  - [x] Record shown URLs after successful response
  - [x] Include score breakdown in response format
  - [x] Add slot assignment to each recommendation

- [x] Task 9: Testing and validation (AC: #1-8)
  - [x] Unit tests for all scoring functions
  - [x] Integration test: Multiple requests return different results
  - [x] Integration test: Recency scoring affects ranking
  - [x] Manual validation: Request recommendations 3x, verify variety
  - [x] Performance test: Verify <60s response time maintained
...
**2025-12-11 - Implementation Plan:**
- Task 1: Add recency scoring with exponential decay to recommendation_filter.py
- Task 2: Create combined scoring function with configurable weights
- Task 3: Implement shown recommendations tracking in Firestore
- Task 4: Add stochastic sampling with temperature-controlled softmax
- Task 5: Implement slot-based rotation strategy (RELEVANCE, SERENDIPITY, STALE_REFRESH, TRENDING)
- Task 6: Enhance query variation with cluster rotation and synonym expansion
- Task 7: Add ranking config documents and MCP tool
- Task 8: Integrate all components into get_reading_recommendations()
...
All acceptance criteria implemented and tested (45 tests passing):

1. **AC#1 - Recency Scoring**: Exponential decay with configurable half-life (default 90 days). Articles older than max_age (default 365 days) filtered out.

2. **AC#2 - Multi-factor Ranking**: Combined score from 4 weighted factors:
   - Relevance (50%): Tavily relevance score
   - Recency (25%): Exponential decay score
   - Depth (15%): Gemini content quality (1-5 normalized)
   - Authority (10%): Author/source credibility from KB

3. **AC#3 - Diversity**:
   - Shown URLs tracked in Firestore with TTL (7 days default)
   - Stochastic sampling with temperature-controlled softmax
   - Novelty bonus for unseen URLs, domain duplicate penalty

4. **AC#4 - Slot-based Rotation**: 5 strategic slots:
   - RELEVANCE (2): Top combined scores
   - SERENDIPITY (1): From different cluster
   - STALE_REFRESH (1): Good quality from less-visited topics
   - TRENDING (1): Highest recency score

5. **AC#5 - Query Variation**:
   - Time-based session seed for hourly rotation
   - Synonym expansion for common terms
   - 8 perspective templates for query phrasing
   - Cluster rotation based on session

6. **AC#6 - Shown Tracking**:
   - `record_shown_recommendations()` persists to Firestore
   - `get_shown_urls()` retrieves within TTL period
   - URLs hashed for document IDs, expires_at for TTL cleanup

7. **AC#7 - Configuration**:
   - `config/ranking_weights` document for factor weights
   - `config/ranking_settings` document for recency, diversity, slots
   - MCP tools: `get_ranking_config`, `update_ranking_config`
   - Weights must sum to 1.0 (validated)

8. **AC#8 - Transparency**:
   - `score_breakdown` shows individual factor scores
   - `ranking_adjustments` shows novelty bonus and domain penalty
   - `slot` and `slot_reason` explain why recommendation was selected
   - `ranking_config` returned in response shows weights/settings used

### File List

**Modified:**
- [recommendation_filter.py](src/mcp_server/recommendation_filter.py): Recency scoring, combined scoring, stochastic sampling, slot assignment
- [recommendation_queries.py](src/mcp_server/recommendation_queries.py): Session seed, synonyms, perspective variation, cluster rotation
- [firestore_client.py](src/mcp_server/firestore_client.py): Ranking config CRUD, shown recommendations tracking
- [tools.py](src/mcp_server/tools.py): Enhanced get_reading_recommendations(), ranking config tools
- [main.py](src/mcp_server/main.py): MCP tool definitions for get_ranking_config, update_ranking_config
- [test_recommendations.py](tests/test_recommendations.py): 25 new tests for Story 3.8

**Firestore Collections:**
- `config/ranking_weights`: Factor weights configuration
- `config/ranking_settings`: Recency, diversity, and slot settings
- `shown_recommendations/{user_id}/items`: TTL-tracked shown URLs
